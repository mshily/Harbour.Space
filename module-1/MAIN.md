...
\[
\quad
\|\nabla f(x)-\nabla f(y)\|\le L\|x-y\|\quad\text{for all }x,y,\ \ \text{with }0<\lambda<\tfrac{2}{L}.
\]
...
$
\textbf{Definition (Strong convexity).}\ 
$
...
$\textbf{Theorem 2 (Convergence of gradient descent with constant step).}$
...
where $\quad q=\max\{|1-\lambda\Lambda|,\ |1-\lambda L|\}.$
